...
function shouldCharge( subscription ) {
    ...
    var isChargeDate = currentDate.isSameDayByTimestamp( new Calendar( nextChargeDate ) ) || isRetryDate;
    return isChargeDate;
}
 ...
 * @returns {void}
 */
function handleSubscriptionRetry( subscription, subToEdit, site ) {
    var retryNumber = site.getCustomPreferenceValue( 'kpSubsRetryNumber' ) ? site.getCustomPreferenceValue( 'kpSubsRetryNumber' ) : site.getCustomPreferenceValue( 'kpRecurringNumberOfRetry' ).value;

    var subRetryCount = subscription.retryCount || 0;
    var newRetryCount = !empty( subscription.retryCount ) ? ( parseInt( subscription.retryCount, 10 ) + 1 ) : 0;

    if ( isRetryEnabled() && subRetryCount < retryNumber && newRetryCount < retryNumber ) {
        var retryFrequency = site.getCustomPreferenceValue( 'kpSubsRetryFrequency' ) ? site.getCustomPreferenceValue( 'kpSubsRetryFrequency' ) : site.getCustomPreferenceValue( 'kpRecurringRetryFrequency' ).value;
                ...
                try {
                    ...
                    if ( orderNo ) {
                        ...
                        if ( subscription.isTrial === 'true' ) {
                            result = SubscriptionHelper.payRecurringOrder( orderNo );
                            if ( !result.error ) {
                            ...
                            result = SubscriptionHelper.createRecurringOrder( orderNo );
